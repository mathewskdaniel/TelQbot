...
from src.bot.handlers.callbacks import get_category_router, get_add_torrents_router, get_list_router, \
    ...
    get_delete_all_router, get_settings_router, get_content_category_router, get_stremio_router

from src.bot.middlewares import UserMiddleware, CustomI18nMiddleware, AuthorizationMiddleware

from src.tasks import torrent_finished, watch_config, seed_completed_torrents
...
async def main(base_path: Path) -> None:
    ...
    await seed_completed_torrents(redis_client, settings)
    ...
    dp.message.middleware(AuthorizationMiddleware(settings))
    dp.callback_query.middleware(AuthorizationMiddleware(settings))
    ...
    dp.include_router(get_content_category_router())
    ...
    dp.include_router(get_stremio_router())
