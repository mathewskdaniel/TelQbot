...
class QbittorrentManager(ClientManager):
    ...
    async def add_magnet(
        self,
        magnet_link: Union[str, List[str]],
        category: str = None,
        save_path: str = None,
    ) -> bool:
        if category == "None":
        ...
        async with self._client() as qbt:
            logger.debug(f"Adding magnet with category {category}, save_path {save_path}")
            result = await qbt.call(
    ...
    async def add_torrent(self, file_name: str, category: str = None, save_path: str = None) -> bool:
        if category == "None":
            category = None

        try:
            async with self._client() as qbt:
                logger.debug(f"Adding torrent with category {category}, save_path {save_path}")
                result = await qbt.call(
                    qbt.client.torrents_add,
                    torrent_files=file_name,
                    category=category,
                    save_path=save_path,
                )
            return result == "Ok."
    ...
    async def get_categories(self):
        async with self._client() as qbt:
            categories = await qbt.call(lambda: qbt.client.torrent_categories.categories)
